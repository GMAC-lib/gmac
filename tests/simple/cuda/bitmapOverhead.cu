...
void compute(const int * globalIn, int * globalOut)
{
    ...
    if (STORES > 2) {
        ...
        for (i = 0; i < STORES - 1; i++) {
            globalOut[index + (i % STRIDE) * THREADS] = shared[threadIdx.x + (i % STRIDE) * THREADS];
        ...
        globalOut[index] = shared[threadIdx.x];
        ...
        globalOut[index + THREADS] = shared[threadIdx.x + THREADS];
    } else if (STORES == 1) {
        globalOut[index] = shared[threadIdx.x];
    }
}
...
doCompute4(const int * in, int * out)
{
    ...
    for (unsigned i = 0; i < REPS; i++) {
        compute<A, B, C, D>   <<<BLOCKS, THREADS>>>(in, out);
...
int main(int argc, char * argv[])
{
    ...
    ret = cudaMalloc(&in, sizeof(int) * THREADS * BLOCKS * STRIDE);
    assert(ret == cudaSuccess);
    ret = cudaMalloc(&out, sizeof(int) * THREADS * BLOCKS * STRIDE);
    assert(ret == cudaSuccess);
