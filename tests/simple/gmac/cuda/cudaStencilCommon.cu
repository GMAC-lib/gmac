...
struct JobDescriptor {
    ...
    float * u3_dev, * u3_host;
    float * u2_dev, * u2_host;
...
do_stencil(void * ptr)
{
	...
	float * v_host = NULL, *v_dev = NULL;
	gmactime_t s, t;

	getTime(&s);

	// Alloc 3 volumes for 2-degree time integration
	assert(cudaMalloc((void **)&descr->u2_dev, descr->size()) == cudaSuccess);
	cudaMemset(descr->u2_dev, 0, descr->size());
	assert(cudaMalloc((void **)&descr->u3_dev, descr->size()) == cudaSuccess);
    cudaMemset(descr->u3_dev, 0, descr->size());

    assert((v_host = (float *)malloc(descr->realSize())) != NULL);
    assert(cudaMalloc((void **) &v_dev, descr->realSize()) == cudaSuccess);

    if(descr->prev != NULL)
        assert((descr->left = (float *)malloc(descr->sliceElems() * STENCIL * sizeof(float))) != NULL);

    if(descr->next != NULL)
        assert((descr->right = (float *)malloc(descr->sliceElems() * STENCIL * sizeof(float))) != NULL);
    ...
    for (size_t k = 0; k < descr->slices; k++) {        
        for (size_t j = 0; j < descr->dimRealElems; j++) {        
            for (size_t i = 0; i < descr->dimRealElems; i++) {        
                ...
                v_host[iter] = VELOCITY;
            }
        }
    }
    assert(cudaMemcpy(v_dev, v_host, descr->realSize(), cudaMemcpyHostToDevice) == cudaSuccess);
    ...
    for (uint32_t i = 1; i <= ITERATIONS; i++) {
        ...
        // Call the kernel
        kernelStencil<32, 8><<<Dg, Db>>>((descr->u2_dev + descr->dimElems * STENCIL + STENCIL),
                                         (descr->u3_dev + descr->dimElems * STENCIL + STENCIL),
                                         (v_dev),
                                         0.08f,
        ...
        if(descr->gpus > 1) {
            ...
            // Send data
            if (descr->prev != NULL) {
                cudaMemcpy(descr->left,
                           descr->u3_dev + STENCIL * descr->sliceElems(),
                           descr->sliceElems() * STENCIL * sizeof(float),
                           cudaMemcpyDeviceToHost);
            }
            if (descr->next != NULL) {
                cudaMemcpy(descr->right,
                           descr->u3_dev + descr->elems() - 2 * STENCIL * descr->sliceElems(),
                           descr->sliceElems() * STENCIL * sizeof(float),
                           cudaMemcpyDeviceToHost);
            }
            ...
            if (descr->prev != NULL) {
                cudaMemcpy(descr->u3_dev + descr->elems() - STENCIL * descr->sliceElems(),
                           descr->prev->right,
                           descr->sliceElems() * STENCIL * sizeof(float),
                           cudaMemcpyHostToDevice);
            }
            if (descr->next != NULL) {
                cudaMemcpy(descr->next->left,
                           descr->u3_dev + descr->elems() - 2 * STENCIL * descr->sliceElems(),
                           descr->sliceElems() * STENCIL * sizeof(float),
                           cudaMemcpyHostToDevice);
            }
    ...
    getTime(&s);
	cudaFree(descr->u2_dev);
	cudaFree(descr->u3_dev);
	cudaFree(v_dev);

    if(descr->prev != NULL) free(descr->left);
    if(descr->next != NULL) free(descr->right);
