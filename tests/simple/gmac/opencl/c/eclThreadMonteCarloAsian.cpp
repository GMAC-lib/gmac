...
void* MonteCarloAsian(void *ThreadID)
{
	...
	assert(error_code == eclSuccess);
	error_code = eclSetKernelArgPtr(kernel, 4,  priceVals[*n]);
	assert(error_code == eclSuccess);
	error_code = eclSetKernelArgPtr(kernel, 5, priceDeriv[*n]);
	assert(error_code == eclSuccess);
...
int main(int argc, char *argv[])
{
	...
	for(unsigned n = 0; n < nIter; n++) {
		...
		assert(refVega != NULL);
		error_code = eclMalloc((void **)&priceVals[n], width * height * 2 * sizeof(cl_float4));
		assert(error_code == eclSuccess);
		error_code = eclMalloc((void **)&priceDeriv[n], width * height * 2 * sizeof(cl_float4));
		assert(error_code == eclSuccess);
	}
	getTime(&t);
	printTime(&s, &t, "Alloc: ", "\n");
	...
	for (unsigned i = 0; i < nIter; i++) {
		...
		eclMemset(priceVals[i], 0, width * height * 2 * sizeof(cl_float4));
		eclMemset(priceDeriv[i], 0, width * height * 2 * sizeof(cl_float4));
	}	
	getTime(&t);
	printTime(&s, &t, "Init: ", "\n");
	...
	for(unsigned n = 0; n < nIter; n++) {
		...
		free(refVega);
		refVega = NULL;
		error_code = eclFree(priceVals[n]);
		assert(error_code == eclSuccess);
		error_code = eclFree(priceDeriv[n]);
		assert(error_code == eclSuccess);
	}
	if(price)
	{
