...
int main(int argc, char *argv[])
{
	...
	getTime(&s);
	// Alloc & init data
	ret = eclMalloc((void **)&randArray, nIter * numSamples * sizeof(cl_float4));
	assert(ret == eclSuccess);
	ret = eclMalloc((void **)&output, nIter * numSamples * sizeof(cl_float4));
	assert(ret == eclSuccess);
	...
	eclMemset(output, 0, nIter * numSamples * 4);
	getTime(&t);
	printTime(&s, &t, "Init: ", "\n");
